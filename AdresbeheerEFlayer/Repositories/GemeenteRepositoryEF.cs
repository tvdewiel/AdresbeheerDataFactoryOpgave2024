...
namespace AdresbeheerEFlayer.Repositories
{
    public class GemeenteRepositoryEF : IGemeenteRepository
    {
        ...
        public Gemeente GeefGemeente(int id)
        {
            try
            {
                return MapGemeente.MapToDomain(ctx.Gemeente.Where(x=>x.NIScode==id).AsNoTracking().FirstOrDefault());
            }
            catch(Exception ex)
            {
        ...
        public List<Gemeente> GeefGemeenten()
        {
            try
            {
                return ctx.Gemeente.Select(x=>MapGemeente.MapToDomain(x)).ToList();
            }
            catch (Exception ex)
            {
        ...
        public void VerwijderGemeente(int id)
        {
            try
            {
                ctx.Gemeente.Remove(new Model.GemeenteEF() { NIScode = id });
                SaveAndClear();
            }
            catch (Exception ex)
            {
